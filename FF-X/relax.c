...

{
  ...
  for ( i = 0; i < gnum_op_conn; i++ ) {      
      gop_conn[i].is_in_naxA = 0;
      gop_conn[i].is_in_axA = 0;
...

{
  ...
  for ( i = 0; i < lnum_E; i++ ) {
    ...
    gop_conn[gef_conn[lE[i]].op].is_in_naxA = 1;
...

{
  ...
  for ( i = 0; i < lnum_E; i++ ) {
    ...
    gop_conn[gef_conn[lE[i]].op].is_in_axA = 1;
...

{
  ...
  if ( !lonly_ax && !lonly_nax ) {
	  ...
	  if ( !gef_conn[gft_conn[index].PC[i]].ch ) {
	      gef_conn[gft_conn[index].PC[i]].ch = 1;
  ...
  if ( lonly_ax ) {
	  ...
	  if ( !gef_conn[gft_conn[index].axPC[i]].ch ) {
	      gef_conn[gft_conn[index].axPC[i]].ch = 1;
  ...
  if ( lonly_nax ) {
	  ...
	  if ( !gef_conn[gft_conn[index].naxPC[i]].ch ) {
	      gef_conn[gft_conn[index].naxPC[i]].ch = 1;
...

{
  ...
  for ( i = 0; i < lnum_ch_F; i++ ) {
    ...
    gft_conn[lch_F[i]].is_goal = 0;
    gft_conn[lch_F[i]].ch = 0;
  }
  lnum_ch_F = 0;
  ...
  for ( i = 0; i < gnum_in_plan_E; i++ ) {
    gef_conn[gin_plan_E[i]].in_plan = 0;
  }
  gnum_in_plan_E = 0;
