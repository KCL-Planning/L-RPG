	...
	for (std::vector<BoundedAtom*>::const_iterator ci = atoms_.begin(); ci != atoms_.end(); ci++)
	{
		...
		for (unsigned int i = 0; i < bounded_atom->getAtom().getArity(); i++)
		{
			...
			if (!supported)
			{
				...
				new_properties->push_back(std::make_pair(&bounded_atom->getAtom().getPredicate(), i));
				...
				std::cout << "The " << i << "th term of the fact: ";
				bounded_atom->print(std::cout, dtg_->getBindings());
				std::cout << " is not captured by a property! Add this one: " << *ps->getProperties()[0] << "!" << std::endl;
