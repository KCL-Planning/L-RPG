...
private:
	...
	// Mapping from a TIMobjectSymbol to the values in the initial state for every 'Range'.
	ElementRanges rngs;
...
public:
	...
	void writePropertySpace(ostream& o) const
	{
		for (vector<ValuesUnion>::const_iterator ci = range.begin(); ci != range.end(); ci++)
		{
			const ValuesUnion& values_union = *ci;
			for (ValuesUnion::const_iterator ci2 = values_union.begin(); ci2 != values_union.end(); ++ci2)
			{
				const Values& values = *ci2;
	
				// Now check all the values.
				for (vector<ValHolder *>::const_iterator ci3 = values.begin(); ci3 != values.end(); ci3++)
				{
					const ValHolder* value_holder = *ci3;
	
					// Print out some values! :D
					assert(value_holder != NULL);
					value_holder->write(o);
					o << ", ";
	
					// Get the property space.
					const PropertySpace* space = value_holder->getSpace();
					assert(space != NULL);
					space->write(o);
					o << " --- " << std::endl;
				}
...
public:
	...
	const SASActionTemplate* getSASActionTemplate(const operator_& op) const;
};
