...
public:
	...
	void write(ostream & o) const
	{
		for (Properties::const_iterator ci = properties.begin(); ci != properties.end(); ci++)
		{
			assert (*ci != NULL);
		}
		o << "{";
...
//	void assembleMutexes(TransitionRule *,Property *);
	...
	const set<TransitionRule *>& getRules() const { return rules; }
};
...
public:
	...
	opType operatorType() const { return opt; };
};
