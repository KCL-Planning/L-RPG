...

{
  ...
  for ( i = 0; i < gnum_operators; i++ ) {
	...
	if ( l->negated ) {
	  gis_deleted[l->fact.predicate] = 1;
	} else {
	  gis_added[l->fact.predicate] = 1;
	}
      }
    }
  }
...

{
  ...
  for ( i = 0; i < gnum_full_initial; i++ ) {
	...
	gis_member[gfull_initial[i].args[0]][t] = 1;
      }
    }
  }
...

{
  ...
  if ( ggoal->connective == TRU ) {
    printf("\nff: goal can be simplified to 1. The empty plan solves it\n\n");
    exit( 1 );
  }
  if ( ggoal->connective == FAL ) {
    printf("\nff: goal can be simplified to 0. No plan will solve it\n\n");
    exit( 1 );
  }
...

{
  ...
  gis_added[gnum_predicates] = 0;
  gis_deleted[gnum_predicates] = 0;
  m = 1;
  ...
  for ( j = 0; j < gnum_operators; j++ ) {
	...
	if ( l->negated ) {
	  tmp->negated = 0;
	  gis_added[gnum_predicates - 1] = 1;
	} else {
	  tmp->negated = 1;
	  gis_deleted[gnum_predicates - 1] = 1;
	}
...

{
  ...
  for ( i = 0; i < gnum_operators; i++ ) {
    ...
    case FAL:
      break;
    default:
      printf("\nwon't get here: non OR, AND, ATOM, 1, 0 in dnf. debug me\n\n");
      exit( 1 );
    }
  }
