...
//		delete new_unprocessed_dtg;
	...
	for (std::vector<DomainTransitionGraph*>::const_iterator ci = getManagableObjects().begin(); ci != getManagableObjects().end(); ++ci)
	{
		delete *ci;
	}
	objects_.clear();
	combined_graph.setId(0);
	addManagableObject(&combined_graph);
