...
//	DTGReachability(const SAS_Plus::DomainTransitionGraphManager& dtg_manager, const SAS_Plus::DomainTransitionGraph& dtg_graph, const TermManager& term_manager, PredicateManager& predicate_manager);
	...
	/**
	 * Find a relaxed plan for the relaxation planning problem generated by performReachabilityAnalysis.
	 * @param bounded_goal_facts The goals to be achieved.
	 * @param predicate_manager The predicate manager.
	 * @param allow_new_goals_added Whenever we need to make a substitution this flag determines how to handle them. If True then instead of adding a
	 * cost to the heuristic we create a new goal by replacing the variable domains of the effect of precondition of the action with the variable domains 
	 * we expect them to have. If False we add the common fact layer at which two objects become true.
	 * @param create_helpful_actions If True we generate helpful actions, otherwise we do not.
	 ...
	 */
	unsigned int getHeuristic(const std::vector< const GroundedAtom* >& bounded_goal_facts, MyPOP::PredicateManager& predicate_manager, bool allow_new_goals_added, bool create_helpful_actions, bool instantiate_actions);
