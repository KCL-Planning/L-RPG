...

{
  ...
  h = (*getHeuristicFunction)( &S, &lcurrent_goals );
  ++(*nr_states_visited);
  ...
  while ( h != 0 ) {
    if ( !search_for_better_state( &S, h, &S_, &h_, getHeuristicFunction ) ) {
      return 0;
    }
    print_state(S_);
	 /*org_h = (*getHeuristicFunction)(&S, &S_);*/
    source_to_dest( &S, &S_ );
    h = h_;
    printf("\n                                %4d            ", h);
	 ++(*nr_states_visited);
...

{
  ...
  while ( 1 ) {  
    ...
    h__ = expand_first_node( h, getHeuristicFunction );
    if ( LESS( h__, h ) ) {
      break;
    }
  }
...

{
  ...
  h_ = (*getHeuristicFunction)( &(lehc_current_start->S), &lcurrent_goals );
...

{
  ...
  for ( i = 0; i < lnum_changed_ehc_entrys; i++ ) {
    ...
    lchanged_ehc_entry[lchanged_ehc_entrys[i]] = 0;
  }
  lnum_changed_ehc_entrys = 0;
...

{
  ...
  add_to_bfs_space( &ginitial_state, -1, NULL, getHeuristicFunction );
  ...
  while ( 1 ) {
    ...
    ++(*nr_states_visited);
	 if (*nr_states_visited % 1000 == 0) fprintf(stderr, "M");
	 else if (*nr_states_visited % 100 == 0) fprintf(stderr, ".");
    ...
    for ( i = 0; i < gnum_naxA; i++ ) {
      ...
      add_to_bfs_space( &S, gnaxA[i], first, getHeuristicFunction );
    }
...

{
  ...
  h = (*getHeuristicFunction)( S, &ggoal_state );
